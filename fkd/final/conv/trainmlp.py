...
#        save_path = save_path),
                 MonitorFilters(
        save_path = 'pretrainfilters_max_1_991616_lr'+str(lr)+"_"+preprocess,
        param='W',layer=0),
                   MonitorFilters(
        save_path = 'pretrainfilters_max_2_991616_lr'+str(lr)+"_"+preprocess,
        param='W',layer=1),
                 MonitorResults(
        mode='full',map_layer=0,#axes_layer=-1,
        save_path = 'pretrainresults_max_1_991616_lr'+str(lr)+"_"+preprocess),
                 MonitorResults(
        mode='axes',map_layer=1,axes_layer=-1,
        save_path = 'pretrainresults_max_2_991616_lr'+str(lr)+"_"+preprocess)]
...
def build_conv(lr,pretrainset,prevalidset,trainset,validset,preprocess):
    ...
    layers = []
    layers.append(get_PosConvLayer('h0',kernel_shape= [9, 9],output_channels=32,crop_border=True,centered=True))
...
#    layers.append(RectifiedLinear(layer_name='h2',dim=96+96,irange=0.1))
